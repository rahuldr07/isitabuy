...
export default function SimpleNav({ variant = "sticky" }: SimpleNavProps) {
  return (
    ...
    >
      <div className="flex h-16 md:h-20 items-center justify-between max-w-[1280px] mx-auto px-6 md:px-8">
        <Link
          href="/"
          aria-label="IsItABuy AI home"
          className="flex min-w-0 items-center gap-2"
        >
          <span className="grid size-9 shrink-0 place-items-center rounded-xl bg-primary-container/10 text-primary-container">
            <ShoppingBag className="size-5" aria-hidden="true" />
          </span>
          <span className="truncate text-xl font-bold text-on-surface tracking-tight">
            IsItABuy AI
          </span>
        </Link>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <Link
            href="/search"
            className="inline-flex h-10 items-center rounded-lg bg-slate-900 px-4 sm:px-5 text-sm font-semibold text-white shadow-sm hover:bg-slate-800 transition-colors"
          >
            Get Started
          </Link>
        </div>
